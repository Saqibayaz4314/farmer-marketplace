...
const EditProduct = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        ...
        // Set product data with existing values
        setProduct({
        ...
        const mainImageUrl = productData.mainImage ? getImageUrl(productData.mainImage) : null;
        const otherImageUrls = (productData.otherImages || productData.images || []).map(img => getImageUrl(img));
        
        setPreviewMain(mainImageUrl);
        setPreviewOthers(otherImageUrls);
  ...
  const handleOtherImagesChange = (e) => {
    ...
    setProduct({ ...product, otherImages: [...product.otherImages, ...files] });
    const newPreviews = files.map((file) => URL.createObjectURL(file));
    setPreviewOthers([...previewOthers, ...newPreviews]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Append main image if new one uploaded
      if (product.mainImage) {
        formData.append('mainImage', product.mainImage);
      }
      
      // Append other images if new ones uploaded
      ...
      if (previewMain && product.mainImage) URL.revokeObjectURL(previewMain);
      previewOthers.forEach(url => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-green-800 font-semibold">Loading product details...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-6 md:px-16">
      ...
      <div className="flex justify-between items-center mb-10">
        <h1 className="text-3xl font-bold text-green-800">Update Product 🌾</h1>
        <button
          onClick={() => navigate(-1)}
